...
float4 PS(PixelInput input) : SV_TARGET
{
	...
	float2 NormalUV = input.vPosition.xy;
	NormalUV.x += time.x * 0.25f;

	float3 Normal = NormalMap.Sample(NormalMapSampler, NormalUV).rgb;

	input.vPosition.xy += Normal.xy * 0.01f;
